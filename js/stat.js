...
window.renderStatistics = function (ctx, players, times) {
  ...
  renderText(ctx, 'Ура вы победили!', TEXT_X, TEXT_Y, '16px PT Mono', '#000');
  renderText(ctx, 'Список результатов:', TEXT_X, TEXT_Y + FONT_SPACE, '16px PT Mono', '#000');
  ...
  for (var i = 0; i < players.length; i++) {
    ...
    renderText(ctx, players[i], cloudSummX, CLOUD_HEIGHT - FONT_SPACE, '16px PT Mono', '#000');
    // вызываем функцию для результата игрока
    renderText(ctx, Math.floor(times[i]), cloudSummX, CLOUD_HEIGHT - (barHeight * times[i]) / maxTime - 35, '16px PT Mono', '#000');
  }
};
